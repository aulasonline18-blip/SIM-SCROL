...
void main() {
  test('manifest protege os guardas reais do app', () {
    ...
    if (!manifestFile.existsSync()) {
      final localGuards = [
        ...
        'lib/sim/lesson/lesson_readiness_resolver.dart',
        'lib/sim/cloud/cloud_queue.dart',
      ];
      for (final path in localGuards) {
  ...
  test(
    'janela de 15, worker, idempotencia, Retry-After, cache e fila continuam protegidos',
    () {
      final dopamine = readRequired(
        'lib/sim/lesson/dopamine_ready_window_engine.dart',
      );
      expectContains(
        dopamine,
      ...
      final readiness = readRequired(
        'lib/sim/lesson/lesson_readiness_resolver.dart',
      );
      expectContains(
        readiness,
        'class LessonReadinessResolver',
        'LessonReadinessResolver',
      );
      ...
      expectContains(
        queue,
        'abstract interface class DurableCloudQueueStorage',
        'CloudQueue',
      );
      expectContains(queue, '_flushingIds', 'CloudQueue');

      final driftQueue = readRequired(
        'lib/sim/cloud/drift_cloud_queue_storage.dart',
      );
      expectContains(
        driftQueue,
        'class DriftCloudQueueStorage',
        'DriftCloudQueueStorage',
      );
      expectContains(
        driftQueue,
        'StudentStateDriftDatabase',
        'DriftCloudQueueStorage',
      );
  ...
  test('sentinela nao autoriza tocar prompt, adendo ou N3', () {
    ...
    if (!manifestFile.existsSync()) {
      expect(File('lib/sim/media/visual_router_n3.dart').existsSync(), isTrue);
      return;
    }
    ...
    expectContains(
      manifest,
      'touch_prompts_adendos_n3',
      'manifest forbidden list',
    );
    expectContains(
      manifest,
      '/root/SIM-SCROL/lib/sim/media/visual_router_n3.dart',
      'manifest protected surfaces',
    );
