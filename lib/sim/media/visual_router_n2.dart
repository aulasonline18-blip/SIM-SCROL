...
enum VisualVerdict { svg, ai, ambiguous, noImage }
...
class VisualN2Result {
  ...
  final VisualPedagogicalRole? pedagogicalRole;
}
...
VisualN2Result classifyVisualByKeywords({
  ...
  if (vt == 'anatomy') {
    return _withN2Log(
      const VisualN2Result(
        ...
        confidence: 0.96,
        pedagogicalRole: VisualPedagogicalRole.realisticReference,
      ),
      topic: topic,
      visualType: visualType,
      imagePrompt: imagePrompt,
  ...
  final negatesPhotoRealism = _negatesPhotoRealism(parts);
  final photoEscape =
      !negatesPhotoRealism &&
      _photoRealismEscape.any((k) => _matchesHint(parts, k));
  ...
  if (lockedHits.isNotEmpty && !photoEscape) {
    return _withN2Log(
      VisualN2Result(
        ...
        confidence: 0.9,
        pedagogicalRole: inferVisualPedagogicalRole(
          topic: topic,
          visualType: visualType,
          imagePrompt: imagePrompt,
        ),
  ...
  final aiHits = _aiHints
      .where((k) => _matchesHint(parts, k))
      .where((k) => !negatesPhotoRealism || !_isPhotoRealismHint(k))
      .toList();
  ...
  if (svgHits.isNotEmpty && aiHits.isEmpty) {
    return _withN2Log(
      VisualN2Result(
        ...
        confidence: _confidenceForHits(svgHits),
        pedagogicalRole: inferVisualPedagogicalRole(
          topic: topic,
          visualType: visualType,
          imagePrompt: imagePrompt,
  ...
  if (aiHits.isNotEmpty && svgHits.isEmpty) {
    return _withN2Log(
      VisualN2Result(
        ...
        confidence: _confidenceForHits(aiHits),
        pedagogicalRole: inferVisualPedagogicalRole(
          topic: topic,
          visualType: visualType,
          imagePrompt: imagePrompt,
...
VisualN2Result _withN2Log(
  ...
  if (kDebugMode) {
    debugPrint(
      ...
      'reason=${result.reason} confidence=${result.confidence.toStringAsFixed(2)} '
      'role=${result.pedagogicalRole?.id ?? 'unknown'} '
      'matched=${result.matched.take(8).join('|')} '
...
double _confidenceForHits(List<String> hits) {
  if (hits.length >= 4) return 0.86;
  if (hits.length >= 2) return 0.78;
  return 0.68;
}

bool _negatesPhotoRealism(String text) {
  return [
    'sem foto',
    'sem imagem realista',
    'sem realismo',
...
bool _isPhotoRealismHint(String hint) {
  return [
    'foto',
    'photo',
    'fotografia',
