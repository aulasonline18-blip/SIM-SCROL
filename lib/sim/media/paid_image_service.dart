...
/// Service-level paid-image state used by this isolated media service.
///
/// The live classroom UI offer model is [LessonPaidImageOffer] on
/// `LessonEventBus`; this distinct name prevents accidental imports of the
/// wrong offer type.
class PaidImageServiceOffer {
  PaidImageServiceOffer({
...
class PaidImageService {
  ...
  final Map<String, PaidImageServiceOffer> _offers = {};
  final _offerController = StreamController<PaidImageServiceOffer>.broadcast();

  /// Stream de ofertas — UI escuta para mostrar lesson_paid_image_offer.dart.
  Stream<PaidImageServiceOffer> get offerStream => _offerController.stream;

  /// Emite oferta de imagem paga. Não debita crédito.
  /// Chamado pelo orchestrator quando S12 retorna generate:true + allowPaid:false.
  PaidImageServiceOffer offer({
    required String lessonKey,
  ...
  PaidImageServiceOffer? getOffer(String offerId) => _offers[offerId];
