...
class DartIoSimHttpTransport implements SimHttpTransport {
  ...
  Future<SimHttpResponse> postJson(
    ...
    try {
      ...
      _debug('[SIM_HTTP] postJson tcp abriu');
      headers.forEach(request.headers.set);
      _debug('[SIM_HTTP] postJson headers ok');
      final encoded = utf8.encode(jsonEncode(body));
      _debug('[SIM_HTTP] postJson body encoded ${encoded.length} bytes');
      request.headers.set('content-length', encoded.length.toString());
      request.add(encoded);
      _debug('[SIM_HTTP] postJson body escrito, fechando');
      final response = await request.close().timeout(timeout);
      _debug('[SIM_HTTP] postJson status ${response.statusCode}');
  ...
  Stream<String> postEventStream(
    ...
    try {
      ...
      _debug('[SIM_HTTP] postEventStream tcp abriu');
      headers.forEach(request.headers.set);
      _debug('[SIM_HTTP] postEventStream headers ok');
      final encoded = utf8.encode(jsonEncode(body));
      _debug('[SIM_HTTP] postEventStream body encoded ${encoded.length} bytes');
      request.headers.set('content-length', encoded.length.toString());
      request.add(encoded);
