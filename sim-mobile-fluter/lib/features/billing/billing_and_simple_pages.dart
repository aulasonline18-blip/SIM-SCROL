...
class _CreditsLabScreenState extends State<CreditsLabScreen> {
  ...
  Future<void> _openPack(String packId) async {
    ...
    final error = await widget.session.startCreditsCheckout(packId);
    if (!mounted) return;
    setState(() {
      loadingPack = null;
      checkoutError = error;
...
class _CheckoutReturnScreenState extends State<CheckoutReturnScreen> {
  Future<CheckoutReturnState>? _future;

  @override
  void initState() {
    super.initState();
    final uri = Uri.tryParse(widget.session.route);
    _future = widget.session.confirmCheckoutReturn(
  ...
  @override
  Widget build(BuildContext context) {
    return FutureBuilder<CheckoutReturnState>(
      future: _future,
      builder: (context, snapshot) {
        final state = snapshot.data;
        final loading = snapshot.connectionState != ConnectionState.done;
        final success = state?.status == CheckoutStatusKind.complete;
